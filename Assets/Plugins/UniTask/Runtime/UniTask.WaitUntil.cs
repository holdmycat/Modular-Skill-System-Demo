...
namespace Cysharp.Threading.Tasks
{
    public partial struct UniTask
    {
        ...
        public static  async UniTask<bool> WaitUntilWithTimeout(Func<bool> predicate, TimeSpan timeout)
        {
            var waitUntilTask = UniTask.WaitUntil(predicate);
            var timeoutTask = UniTask.Delay(timeout);

            int completedIndex = await UniTask.WhenAny(waitUntilTask, timeoutTask);

            if (completedIndex == 1) // timeoutTask 的索引是 1
            {
                return false; // 超时
            }
