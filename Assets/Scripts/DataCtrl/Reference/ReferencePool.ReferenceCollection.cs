        ...
        private sealed class ReferenceCollection
        {
            ...
            public IReference Acquire()
            {
                ...
                lock (m_References)
                {
                    if (m_References.Count > 0)
                    {
                        var item = m_References.Dequeue();
                        //log.DebugFormat("Reference ,Get Queue reference {0}, count is {1}", m_ReferenceType.FullName, m_References.Count);
                        return item;
                    }
                }

                m_AddReferenceCount++;
                //log.DebugFormat("Reference ,Create new reference {0}, count is {1}", m_ReferenceType.FullName, m_References.Count);
